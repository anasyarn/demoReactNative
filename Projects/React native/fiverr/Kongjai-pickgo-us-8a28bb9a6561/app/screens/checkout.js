...
import { colors, Icon, List, ListItem } from 'react-native-elements';
...
class _ReserveATableButton extends Component {
  sum(key) {
    let res = this.props.app.cart.reduce(
      (a, b) => parseInt(a) + parseInt(b[key] || 0) * parseInt(b['count'] || 1),
      0
    );
    return (
      res +
      this.props.app.cart.reduce(
        (a, b) => parseInt(a) + parseInt(b['mainPrice'] || 0),
        0
      )
    );
  }
  ...
  updateCart(item) {
    ...
    // this.props.updateCart(data.filter((d) => d.id !== item.id));
  }
  ...
  render() {
    ...
    const getTotal = (data) => {
      let amount = 0;
      data.forEach((d) => (amount = amount + d.total_amount));
      return amount;
    };
    const { navigate } = this.props.navigation;
          ...
          <>
            ...
            <View style={{ padding: 10, backgroundColor: '#fff' }}>
              ...
              >
                ...
                <CurrFormat value={getTotal(app.cart)} />
              </View>
              <View
                style={{
                  ...styles.tableRow,
              ...
              >
                <Text style={styles.tableCell}>{i18nT('checkout.tax')}</Text>
                <CurrFormat value={0 / 100} />
              </View>
              <View
                style={{
              ...
              >
                <Text style={styles.tableCell}>{i18nT('checkout.tips')}</Text>
                <CurrFormat value={0 / 100} />
              </View>
              <View
                style={{
            ...
            {/* <FlatList
              ...
              renderItem={({ item }) => (
                ...
                >
                  ...
                  >
                    <Text>{`x${item.qty}` + '  ' + item.menu_name}</Text>
                    <View
...
const ReserveATableButton = connect(
  ...
  { reserveATable, createReservation, companyShowData, i18nT, updateCart }
        ...
        {app.loading ? (
          ...
          <View style={{ flex: 1 }}>
            ...
            >
              ...
              >
                <TouchableOpacity>
                  <Text style={{ color: colors.primary }}>Reserve Table</Text>
                </TouchableOpacity>
                <TouchableOpacity>
                  <Text style={{ color: colors.primary }}>Write review</Text>
                </TouchableOpacity>
              </View>
              <PromoList />
            </Animated.View>
                ...
                renderItem={({ item }) => {
                  return (
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        console.log('Datatatatatata: ', item);
                        navigate('ItemSelect', {
                          data: item,
                          res_id: this.props.navigation.state.params.res_id,
                          menu: item,
                          cid: this.props.navigation.state.params.res_id,
                        });
                      }}
                    >
                      <View style={{ flex: 3 }}>
                  ...
                  <View style={{}}>
                    ...
                    {!hasOrder && !hasReservations && (
                      <Fragment>
                        ...
                        {hasCardData && !hideReservation && (
                          <ReserveATableButton {...props} showButton />
                        )}
                      </Fragment>
                    )}
                    <View style={{ flex: 2 }} />
